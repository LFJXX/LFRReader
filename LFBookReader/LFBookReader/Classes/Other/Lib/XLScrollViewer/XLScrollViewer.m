...
@property (nonatomic, assign) NSInteger startContentOffsetX; //开始滑动时的坐标
@property (nonatomic, assign) NSInteger willEndContentOffsetX; // 将要结束的坐标
...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.xl_views = [NSArray array];
        self.xl_buttonNames = [NSArray array];
        
            }
    return self;
}
- (void)createwithViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose
{
...
#warning 改了
        if (!self.xl_views) {
            UIView *temp =[[UIView alloc]initWithFrame:(CGRect){{screen_width*i, -64},self.scroll2.frame.size}];
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    NSInteger endContentOffsetX = scrollView.contentOffset.x;
    if (endContentOffsetX > _willEndContentOffsetX && _willEndContentOffsetX > _startContentOffsetX) {//画面从左往右移动，后一页
        
        // 监测偏移量请求数据
        [self.delegate requestData:scrollView];
    }
