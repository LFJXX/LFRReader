...
#define kItemCiBa           @"词霸"
#define kItemRead           @"朗读"
#define kItemNotation       @"批注"
#define kItemShare          @"分享"
...
- (void)showMenuUI{
    ...
    if ([self becomeFirstResponder]) {
        ...
        UIMenuItem *menuItemLine = [[UIMenuItem alloc] initWithTitle:kItemLine action:@selector(line:)];
        UIMenuItem *menuItemNotation = [[UIMenuItem alloc] initWithTitle:kItemNotation action:@selector(notation:)];
        UIMenuItem *menuItemCopy = [[UIMenuItem alloc] initWithTitle:kItemCopy action:@selector(copyword:)];
        UIMenuItem *menuItemShare = [[UIMenuItem alloc] initWithTitle:kItemShare action:@selector(share:)];
...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
   //|| action == @selector(highLight:)
    if (action == @selector(line:) || action == @selector(notation:) || action == @selector(copyword:) || action == @selector(share:)) {
        return YES;
    }
    return NO;
}
...
- (void)line:(id)sender{
    ...
    NSString *text = [self.text substringWithRange:NSMakeRange(selectedRange.location, selectedRange.length)];
    [self resetting];
}
#pragma mark 批注
- (void)notation:(id)sender{
     NSString *text = [self.text substringWithRange:NSMakeRange(selectedRange.location, selectedRange.length)];
    if ([self.delegate respondsToSelector:@selector(notationDidSeletedString:ReaderView:)] ) {
        [self.delegate notationDidSeletedString:text ReaderView:self];
    }
    [self resetting];
}
#pragma mark 分享
- (void)share:(id)sender{
    if ([self.delegate respondsToSelector:@selector(shareDidDidSeletedReaderView:)]) {
        [self.delegate shareDidDidSeletedReaderView:self];
    }
   [self resetting];
}
