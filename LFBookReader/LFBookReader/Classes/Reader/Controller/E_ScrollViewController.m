...
//        [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:UIStatusBarAnimationFade];
    ...
    _settingToolBar.titleLable.text = @"悲伤逆流成河";
    [self changeMarkColor];
...
- (void)changeMarkColor{
    ...
    if ([E_CommonManager checkIfHasBookmark:range withChapter:[E_ReaderDataSource shareInstance].currentChapterIndex]) {
        _settingToolBar.markBtn.selected = YES;
    }else{
        _settingToolBar.markBtn.selected = NO;
    }
    
    if (_settingToolBar.markBtn.selected == YES) {
        
        [_settingToolBar.markBtn setTitleColor:[UIColor redColor] forState:0];
        
    }else{
        
        [_settingToolBar.markBtn setTitleColor:[UIColor whiteColor] forState:0];
    }
    _settingToolBar.markBtn.titleLabel.font = [UIFont systemFontOfSize:16.0];
}
...
- (void)doMark{
    ...
    _settingToolBar.markBtn.selected = !_settingToolBar.markBtn.selected;
    if (_settingToolBar.markBtn.selected == YES) {
        [_settingToolBar.markBtn setTitleColor:[UIColor redColor] forState:0];
    }else{
        [_settingToolBar.markBtn setTitleColor:[UIColor whiteColor] forState:0];
